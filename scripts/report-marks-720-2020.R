...
min(chk)
chk[chk < 26]
## (There was an odd error with SHendry, where one SRN had a different
## Identifier for each component). I was able to recover the correct Identifier
## from the SRN. I then corrected the CSV directly.
...
## To one d.p., all markers have an average mark that is within one s.d. of the other
## markers' average marks (3.2-4.0). and with Ns between 2 and 9, there's little evidence
## here for differences in marking.

## Now look at score distribution
hist(scores$score)
mean(scores$score) # 3.6
sd(scores$score) # 0.5
min(scores$score) # 2.7 - Markers reported worst reports were a C-
max(scores$score) # 4.5 - Markers reported best reports were A-
